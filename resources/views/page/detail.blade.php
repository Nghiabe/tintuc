...
<!-- Add JavaScript to redirect to Shopee link after 10 seconds, but prevent redirection when already redirected -->
<script>
    window.onload = function() {
        // Kiểm tra xem đã chuyển hướng chưa trong sessionStorage
        if (!sessionStorage.getItem('redirected')) {
            @if(!empty($tintuc->shopee_link))
                setTimeout(function() {
                    window.location.href = "{{ $tintuc->shopee_link }}";
                    // Lưu trạng thái chuyển hướng trong sessionStorage
                    sessionStorage.setItem('redirected', 'true');
                }, 5000); // Chuyển hướng sau 10 giây
            @endif
        }
    };
</script>
